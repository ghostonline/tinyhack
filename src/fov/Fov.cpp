...
MyVisibility::MyVisibility(BlockLightFunc blocksLight, SetVisibleFunc setVisible, GetDistanceFunc getDistance)
{
...
void MyVisibility::Compute(LevelPoint origin, fov::OctantSet octants_to_compute, int rangeLimit)
{
    _setVisible(origin.x, origin.y);
    for(uint octant=0; octant<8; octant++)
    {
        if (octants_to_compute[octant])
        {
...
void MyVisibility::Compute(uint octant, LevelPoint origin, int rangeLimit, uint x, Slope top, Slope bottom)
{
    ...
    for(; x <= (uint)rangeLimit; x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
    {
        ...
        for(uint y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
        {
            if(rangeLimit < 0 || GetDistance((int)x, (int)y) <= rangeLimit) // skip the tile if it's out of visual range
            {
                ...
                if(x != (uint)rangeLimit) // but don't bother adjusting them if this is the last column anyway
                {
                    if(isOpaque)
                    {
                        ...
                        {                  // so adjust the bottom vector upward and continue processing it in the next column
                            ...
                            {                       // created by adjusting the bottom is only valid if that's the case
                                ...
                                if(y == bottomY) { bottom = Slope(ny, nx); break; } // don't recurse unless necessary
                                else Compute(octant, origin, rangeLimit, x+1, top, Slope(ny, nx));
                            }
